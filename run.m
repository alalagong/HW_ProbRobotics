...
for t = 1:numSteps
  ...
  if usePF
    ...
    figure(GLOBAL_FIGURE);
     plotSamples(particles','b');
     plotcov2d(mu(1), mu(2), sigma(1:2,1:2), [0.9, 0.2, 0.3], 4);
    end
  else
    ...
    [mu, sigma, predMu, predSigma, zHat, pOfZ, G, V, H, K] = ekfUpdate(mu, sigma, ...
     ...
     if (doPlot)
      figure(GLOBAL_FIGURE);
      plotcov2d(mu(1),mu(2), sigma(1:2,1:2), [0.2, 0.3, 0.7], 10);
     end
  end
...
%    disp('Currently ignoring filter values');
  ...
  if (doPlot) 
%     positionDriftExample = sqrt(sum( (data(1:t,8:9) - muList(1:t,1:2)).*(data(1:t,8:9) - muList(1:t,1:2)), 2));
%     figure(ERROR_FIGURE); clf; plot(positionDriftExample);
  end
...
%     fflush(stdout);
  ...
  mahalanobis(t) = errors(t,:)*pinv(sigma)*errors(t,:)';

  if t == 150
        disp('numStep = 120,paused ......')
        pause(10);
  end
